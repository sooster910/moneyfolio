...
const ExpenseChart = (props) => {
  ...
  const total = props.expense.map((expense) => {
    return expense.amount
  }).reduce((sum, value) => {
    return sum + parseFloat(value.toString())
  }, 0)

  const categoryData = [foodCategoryTotal, clothingCategoryTotal, educationCategoryTotal,
    entertainmentCategoryTotal, electronicCategoryTotal, travelCategoryTotal, utilitiesCategoryTotal,
    transportationCategoryTotal, savingsCategoryTotal, debtCategoryTotal, otherCategoryTotal]
 
  const categoryPercentage =  categoryData.map((data)=>{return Math.round((data/total)*100)});


 const categoryName = ['food', 'clothing', 'education&Training', 'entertainment',
    'electronic', 'travel', 'utilities', 'transportation', 'savings', 'debt', 'other']
